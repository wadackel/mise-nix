...
- **`sources.json` is treated as generated.** Prefer `just update` over hand edits. A failure opens an issue labelled `update-failed`, or rewrites the title and body of the one already open — it never adds a comment, so the only notification is the initial one. The title carries the consecutive failure count (`(×N)`) so the severity is visible from the issue list.
